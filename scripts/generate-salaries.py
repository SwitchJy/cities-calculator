...
def generate_salary_data():
    ...
    for i, hero in enumerate(heroes):
        ...
        # 根据英雄的地位设定基础工资范围
        # 首领级（宋江等）：20000-30000
        # 高级将领（林冲、武松等）：15000-25000
        # 中级将领：10000-18000
        # 普通好汉：6000-12000

        if i < 5:  # 前5位首领
            base_salary = random.randint(20000, 30000)
        elif i < 20:  # 高级将领
            base_salary = random.randint(15000, 25000)
        elif i < 50:  # 中级将领
            base_salary = random.randint(10000, 18000)
        else:  # 普通好汉
            base_salary = random.randint(6000, 12000)
...
# 保存为Excel文件
output_file = 'public/salaries-heroes.xlsx'
df.to_excel(output_file, index=False)

print(f"\n成功生成 {len(heroes)} 位英雄的工资数据！")
print(f"总共 {len(salary_data)} 条记录")
